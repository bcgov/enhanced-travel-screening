...
app.post(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    const messages = [
      `Processing PHAC submission of ${phacItems.length} items`,
    ];
    try {
      await phacCollection.insertMany(phacItems, { ordered: false });
      results.successful = phacItems.reduce(
        (a, v) => ({ ...a, [v.covid_id]: v.id }),
        {}
      );
    } catch (error) {
      try {
...
app.get(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    if (!formItem)
      return res.status(404).json({ error: `No submission with ID ${id}` });
...
app.get(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    if (forms.length === 0)
      return res
        .status(404)
        .json({ error: `No traveller found with last name ${lname}` });
