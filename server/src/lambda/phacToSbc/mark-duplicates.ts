 ...
 * @param {*} item
 */
const merge = (array, item) =>
  flatten([...array, item]).filter((value, index, self) => self.indexOf(value) === index);
...
const createEtsKeys = ets => {
  ...
  for (const etsRecord of ets) {
    ...
    const addresses = [etsRecord.address, etsRecord.isoAddress];

    const derivedAddressArrivalDateKeys = [];
    for (let i = 0; i < addresses.length; i += 1) {
      derivedAddressArrivalDateKeys.push(
        `${transformAddress(addresses[i])}${cleanString(etsRecord.arrivalDate)}`
      );
    }
...
const markDuplicates = async (etsCollection: Collection, phacCollection: Collection<PhacEntry>) => {
    ...
    .aggregate([
      {
        $addFields: {
          parsed_arrival_date: {
            $dateFromString: { dateString: '$arrival_date', timezone: '-0700' },
          },
        },
      },
      {
  ...
  for (const phacEntry of phac) {
    ...
    phacPhones.forEach(item => {
      if (item) {
        phacEntryKeys.push(item);
      }
    });
    if (phacEntry.derivedTravellerKey != null) phacEntryKeys.push(phacEntry.derivedTravellerKey);
    phacEntryKeys.push(
      `${transformAddress(phacEntry.address_1)}${cleanString(phacEntry.arrival_date)}`
    );
    ...
    for (const phacKey of phacEntryKeys) {
      ...
      if (phacKeys[phacKey] && phacKeys[phacKey] !== covidId) {
        ...
        if (
          compareDates(
            phacArrivalDate,
            phac.find(item => item.covid_id === covidId).arrival_date,
            7
          )
        ) {
  ...
  if (process.env.DB_WRITE_SERVICE_DISABLED === 'true') {
    logs.push(
      'DB_WRITE_SERVICE_DISABLED is true. Skipped writing of duplicateIds on serviceBCTransactions.'
    );
  }
