...
describe('Server V1 Auth Endpoints', () => {
  ...
  it('Login using username and password, receive JWT', async () => {
    const res = await request.agent(app).post(loginEndpoint).send({
      username: 'username',
      password: 'password',
    });
  ...
  it('Validate JWT, receive 200', async () => {
    const resLogin = await request.agent(app).post(loginEndpoint).send({
      username: 'username',
      password: 'password',
    });
  ...
  it('Validate wrong JWT, receive 401', async () => {
    const res = await request.agent(app).get(validateEndpoint).set({
      Accept: 'application/json',
      'Content-type': 'application/json',
