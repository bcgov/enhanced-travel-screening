...
const uniquePropsFromObjectArray = (array) => {
...
const filterEmptyArraysAndObjects = (object) => Object.keys(object)
  .filter((k) => { // Determine property type from objects keys and filter empty objects and arrays
    if (typeof object[k] === 'object' && object[k] != null) return Object.keys(object[k]).length > 0;
...
const toCsvString = (data) => new Promise((resolve, reject) => {
...
const fromCsvString = (csv) => new Promise((resolve, reject) => {
  const data = [];
...
module.exports = { toCsvString, fromCsvString };
