...
job-create:
	...
	@oc process -f openshift/test.job.yml -p NAMESPACE=$(OS_NAMESPACE) APP_NAME=$(APP_NAME) SSH_USERNAME=$(AWS_SSH_USERNAME) SSH_KEY=$(AWS_SSH_KEY) SSH_HOST=$(AWS_SSH_HOST) DB_SERVER=$(AWS_DB_SERVER) DB_USER=$(AWS_DB_USER) DB_PASSWORD=$(AWS_DB_PASSWORD) DB_NAME=$(AWS_DB_NAME) SLACK_ENDPOINT=$(SLACK_ENDPOINT) SBC_USER=$(SBC_USER) SBC_PASSWORD=$(SBC_PASSWORD) SBC_SECRET=$(SBC_SECRET) | oc apply -f -
